...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!target) {
      sendMessage(
        message.channel,
        "Please Provide Someone To Duel: Usage- duel <thier @> <bet?>"
      );
      return;
    }
    let bet = args[1];
    ...
    db.add(
      `duelbet_${message.guild.id}_${message.member.id}_${target.id}`,
      bet
    );
    ...
    var embed = new Discord.MessageEmbed()
      .setTitle(
        `${message.member.user.tag} Has Sent A Duel Request To ${targetUser}`
      )
      .setDescription(
