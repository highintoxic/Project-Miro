...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (!target) {
            sendMessage(message.channel, "Please Make Sure That Person Is Dueling You: Usage- acceptduel <thier @>")
            return;
        }

        let bet = db.fetch(`duelbet_${message.guild.id}_${message.member.id}_${message.member.id}`)
        ...
        let duel = db.fetch(`duel_${message.guild.id}_${target.id}_${message.member.id}`) 

        if (duel) {
            sendMessage(message.channel, `Accepted Duel From <@${target.id}>`)
            db.delete(`duel_${message.guild.id}_${target.id}_${message.member.id}`)
            let senderValue = db.fetch(`armor_${message.guild.id}_${message.member.id}.armor`)
            let targetValue = db.fetch(`armor_${message.guild.id}_${target.id}.armor`)
            if (!senderValue) {
                senderValue = 20
            }
            if (!targetValue) {
            ...
            let senderDefence = senderValue + senderWeapon.senderOffset + senderWeapon.targetOffset
            let targetDefence = targetValue + targetWeapon.senderOffset + targetWeapon.targetOffset
            ...
            await sleep(5000)
            const bet = db.fetch(`duelbet_${message.guild.id}_${target.id}_${message.member.id}`)
            if (randomNumber == 69) {
                var rollingX = new Discord.MessageEmbed()
                    .setTitle(`Random Number Was 69 You Both Get Half Of The Bet (${bet/2} Coins)`)
                    .setDescription(`${message.member.user.tag} Items \nDefence: ${senderDefence} \nArmor ${senderArmor.name} \nWeapon: ${senderWeapon.name} \n\n${target.user.tag} Items \nDefence: ${targetDefence} \nArmor ${targetArmor.name} \nWeapon: ${targetWeapon.name}`)
            ...
            sendMessage(message.channel, `Couldnt Find A Duel Request From <@${target.id}>`)
            return;
        }
    }
}