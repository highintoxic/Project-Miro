...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    const mention = message.mentions.members.first() || message.member
        ...
        const background = await Canvas.loadImage('https://wallpapercave.com/wp/wp3597497.jpg');
        ...
        ctx.fillText(`User: ${mention.user.tag}`, canvas.width / 2.5, canvas.height / 3.5);
        ...
        if (!avatar){
          ...
          avatar = await Canvas.loadImage(avatarView.displayAvatarURL({ format: 'jpg' }));
        }
